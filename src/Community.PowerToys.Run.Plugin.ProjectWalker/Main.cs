...
using Community.PowerToys.Run.Plugin.PowerToysRun.ProjectWalker.Factories;
        ...
        try
        {
            ...
            return ConfigService.Instance.Config.FolderStructureType switch
            {
                ...
                _ => throw new ConfigurationErrorsException(
                    $"Unsupported value for FolderStructureType {ConfigService.Instance.Config.FolderStructureType} (int value: {(int)ConfigService.Instance.Config.FolderStructureType})")
            };
        }
        catch (Exception ex)
        {
